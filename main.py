...
def main(): #{
	...
	for Y in range(IMAGE.height): #{								#Loop through each vertical line in image.
		...
		Serial.write(bytes('\x24', 'utf-8'));							#Write terminator for odd bits to serial. Per documentation.
		
		loop(LINES[1:420:2]);									#Jump to loop routine with even bits from bit array.
		if Y == IMAGE.height - 1:	Serial.write(bytes('\x29', 'utf-8'));			#If at end of image, write terminator exitting graphics mode.
		else:				Serial.write(bytes('\x21', 'utf-8'));			#If not, write standard terminator to serial port. Per documentation.
	#}												#End vertical loop.
